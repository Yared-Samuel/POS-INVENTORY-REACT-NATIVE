...
import { getDailyPurchase } from "../redux/features/reports/reportSlice";
import { Table, Row, Rows } from "react-native-table-component";
import Header from "../components/Header";

const PruchaseScreen = () => {

    const dispatch = useDispatch();
    const { dailyPurchases, isLoading, isError, message } = useSelector(
      (state) => state.report
    );
  ...
  return (
    ...
    <ScrollView>
      ...
      <Header title="Daily Purchase - የቀን ግዢ" />
      <ScrollView style={{ marginHorizontal: 20 }}>
        <Table borderStyle={{ borderWidth: 2, borderColor: "#c8e1ff" }}>
          <Row
            data={["Date / ቀን", "Daily Sales / የእለት ግዢ"]}
            style={styles.head}
        ...
        {endIndex < dailyPurchases.length && ( // Show "Load More" button if there's more data
